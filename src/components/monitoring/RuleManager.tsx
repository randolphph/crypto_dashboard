...
export function RuleManager({ monitor, catalog }: { monitor: CryptoSentryMonitor; catalog: IntegrationCatalog }) {
  ...
  return (
    <Card>
      <CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><BellPlus className="size-4" />告警规则</CardTitle><p className="mt-1 text-xs text-muted-foreground">设置什么情况下需要告警，以及通过哪些 Telegram 目标通知。</p></div><Button size="sm" onClick={() => setCreating(true)}><Plus />添加规则</Button></div></CardHeader>
      <CardContent className="space-y-2">
        ...
        {rules.map((rule) => <div key={rule.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{rule.name}</p><Badge variant={rule.enabled ? 'outline' : 'secondary'}>{rule.enabled ? '已启用' : '已停用'}</Badge><Badge variant="secondary">{rule.conditions.length} 条 · {rule.combinator === 'and' ? '全部满足' : '任一满足'}</Badge><Badge variant="outline">{SEVERITY_LABEL[rule.severity]}</Badge></div><div className="mt-2 space-y-1">{rule.conditions.map((condition, index) => <p key={`${condition.metric}-${index}`} className="text-xs text-foreground/80"><span className="text-muted-foreground">条件 {index + 1}：</span>{conditionSummary(condition, availableMetrics.find((metric) => metric.id === condition.metric), monitor)}</p>)}</div><p className="mt-2 text-xs text-muted-foreground">{rule.durationSeconds === 0 ? '满足即触发' : `持续 ${formatSeconds(rule.durationSeconds)}后触发`} · {rule.cooldownSeconds === 0 ? '不限制重复提醒' : `${formatSeconds(rule.cooldownSeconds)}内不重复提醒`}</p><p className="mt-1 text-xs text-muted-foreground">通知：{rule.notificationIntegrationIds?.length ? rule.notificationIntegrationIds.map((id) => telegramQuery.data?.find((integration) => integration.id === id)?.name ?? id).join('、') : '仅站内告警'}</p></div><div className="flex gap-1"><Button size="sm" variant="ghost" onClick={() => setEditing(rule)}><Pencil />编辑</Button><Button size="sm" variant="ghost" onClick={() => updateRule.mutate({ id: rule.id, patch: { enabled: !rule.enabled } })}><CircleOff />{rule.enabled ? '停用' : '启用'}</Button><Button size="icon-sm" variant="ghost" className="text-destructive" aria-label={`删除规则 ${rule.name}`} onClick={() => { if (window.confirm(`删除规则“${rule.name}”？`)) deleteRule.mutate(rule.id); }}><Trash2 /></Button></div></div>)}
      </CardContent>
